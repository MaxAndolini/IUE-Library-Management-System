...
public class ManageBorrowerController
{
    ...
    public void initialize() throws SQLException {
        ...
        try {
            table.getItems().addAll(conn.getSearchBorrowedBooks(null, "All", null, null, 2, 0));
    ...
    void borrowbookButtonAction(ActionEvent aevent) throws IOException {
        Parent home_page_parent = FXMLLoader.load(Main.class.getResource("/gui/admin/BorrowBook.fxml"));
        Scene home_page_scene = new Scene(home_page_parent);
        Stage app_stage = (Stage) ((Node) aevent.getSource()).getScene().getWindow();
        app_stage.setScene(home_page_scene);
    ...
    void mbRefreshButtonAction(ActionEvent event) {
        ...
        try {
            table.getItems().addAll(conn.getSearchBorrowedBooks("", "All", null, null, 2, 0));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    void mbSearchButtonAction(ActionEvent event) {
        ...
        table.getItems().clear();
        try {
            table.getItems().addAll(conn.getSearchBorrowedBooks(sbox.getText(), mbstype.getValue(), (mbsfirstdate.getValue() == null) ? null : Timestamp.valueOf(mbsfirstdate.getValue().atTime(0, 0)), (mbsseconddate.getValue() == null) ? null : Timestamp.valueOf(mbsseconddate.getValue().atTime(0, 0)), 2, 0));
        } catch (SQLException e) {
            e.printStackTrace();
        }
