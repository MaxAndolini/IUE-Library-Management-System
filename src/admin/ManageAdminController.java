...
public class ManageAdminController
{
    ...
    @FXML
    private Button masearchbutton;

    @FXML
    private Button marefreshbutton;

    @FXML
    private Button madeletebutton;

    @FXML
    private ComboBox<String> matype;
    ...
    public void initialize() throws SQLException {
        ...
        matype.getItems().addAll("Search by ID", "Search by Username", "Search by First Name", "Search by Last Name");
        matype.getSelectionModel().selectFirst();
        madeletebutton.setVisible(false);
        ...
        try {
            table.getItems().addAll(conn.getSearchUsers("", "All", 2));
        } catch (SQLException e) {
            e.printStackTrace();
        }

        totaladmin.setText("[Total Admin: " + table.getItems().size() + "]");
    ...
    void onTableClicked(MouseEvent event) {
        madeletebutton.setVisible(true);
    }
    ...
    @FXML
    void maddadminButtonAction(ActionEvent aevent) throws IOException {
        Parent home_page_parent = FXMLLoader.load(Main.class.getResource("/gui/admin/AddAdmin.fxml"));
        Scene home_page_scene = new Scene(home_page_parent);
        Stage app_stage = (Stage) ((Node) aevent.getSource()).getScene().getWindow();
        app_stage.setScene(home_page_scene);
    ...
    public void changeColor(String color) {
        if(!color.equals("")) {
            ...
            masearchbutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            marefreshbutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            matype.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            madeletebutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            maddadminbutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            totaladmin.setFill(Paint.valueOf(color));
        }
    }

    @FXML
    void maRefreshButtonAction(ActionEvent event) {
        DBConnectionClass conn = new DBConnectionClass();
        matype.getSelectionModel().selectFirst();
        sbox.clear();
        madeletebutton.setVisible(false);
    ...
    @FXML
    void maSearchButtonAction(ActionEvent event) {
        DBConnectionClass conn = new DBConnectionClass();
        table.getItems().clear();
        try {
            table.getItems().addAll(conn.getSearchUsers(sbox.getText(), matype.getValue(), 2));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    @FXML
    void maDeleteButtonAction(ActionEvent event) {
        if(table.getSelectionModel().getSelectedItem() != null) {
            DBConnectionClass conn = new DBConnectionClass();
            UserInfo user = table.getSelectionModel().getSelectedItem();
            conn.deleteUser(user.getID().intValue());
            marefreshbutton.fire();
        } else madeletebutton.setVisible(false);
    }
}
