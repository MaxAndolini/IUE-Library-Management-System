...
public class ManageLibrarianController
{
    ...
    @FXML
    private Button mlrefreshbutton;

    @FXML
    private Button mldeletebutton;

    @FXML
    private ComboBox<String> mltype;
    ...
    @FXML
    private Text totallibrarian;

    @FXML
    private Button maddlibrarianbutton;
    ...
    public void initialize() throws SQLException {
        ...
        mltype.getItems().addAll("Search by ID", "Search by Username", "Search by First Name", "Search by Last Name");
        mltype.getSelectionModel().selectFirst();
        mldeletebutton.setVisible(false);
        ...
        try {
            table.getItems().addAll(conn.getSearchUsers("", "All", 1));
        } catch (SQLException e) {
            e.printStackTrace();
        }

        totallibrarian.setText("[Total Librarian: " + table.getItems().size() + "]");
    ...
    void onTableClicked(MouseEvent event) {
        mldeletebutton.setVisible(true);
    }
    ...
    @FXML
    void maddlibrarianButtonAction(ActionEvent aevent) throws IOException {
        Parent home_page_parent = FXMLLoader.load(Main.class.getResource("/gui/admin/AddLibrarian.fxml"));
        Scene home_page_scene = new Scene(home_page_parent);
        Stage app_stage = (Stage) ((Node) aevent.getSource()).getScene().getWindow();
        app_stage.setScene(home_page_scene);
    ...
    public void changeColor(String color) {
        if(!color.equals("")) {
            ...
            mlsearchbutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            mlrefreshbutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            mltype.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            mldeletebutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            maddlibrarianbutton.setStyle("-fx-background-color: "+ color.replace("0x", "#"));
            totallibrarian.setFill(Paint.valueOf(color));
        }
    }

    @FXML
    void mlRefreshButtonAction(ActionEvent event) {
        DBConnectionClass conn = new DBConnectionClass();
        mltype.getSelectionModel().selectFirst();
        sbox.clear();
        mldeletebutton.setVisible(false);
    ...
    @FXML
    void mlSearchButtonAction(ActionEvent event) {
        DBConnectionClass conn = new DBConnectionClass();
        table.getItems().clear();
        try {
            table.getItems().addAll(conn.getSearchUsers(sbox.getText(), mltype.getValue(), 1));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    @FXML
    void mlDeleteButtonAction(ActionEvent event) {
        if(table.getSelectionModel().getSelectedItem() != null) {
            DBConnectionClass conn = new DBConnectionClass();
            UserInfo user = table.getSelectionModel().getSelectedItem();
            conn.deleteUser(user.getID().intValue());
            mlrefreshbutton.fire();
        } else mldeletebutton.setVisible(false);
    }
}
