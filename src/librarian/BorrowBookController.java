...
public class BorrowBookController
{
    ...
    void AddButtonAction(ActionEvent event) throws SQLException {
        if(!abookid.getText().isEmpty() && !ausername.getText().isEmpty()) {
            if(StringUtils.isStrictlyNumeric(abookid.getText())) {
                DBConnectionClass conn = new DBConnectionClass();
                if(conn.bookidCheck(Integer.parseInt(abookid.getText()))) {
                    if(conn.bookstatusCheck(Integer.parseInt(abookid.getText()))) {
                        if(conn.usernameCheck(ausername.getText())) {
                            if(conn.getAbilityLevel(conn.getID(ausername.getText())) == 0) {
                                invalid.setText("Info: Book borrowed successfully.");
                                conn.borrowBook(Integer.parseInt(abookid.getText()), ausername.getText());
                                abookid.clear();
                                ausername.clear();
                            } else invalid.setText("Error: Only users can borrow books.");
                        } else invalid.setText("Error: The username not found.");
                    } else invalid.setText("Error: The book is not in the library.");
                } else invalid.setText("Error: The book not found.");
            } else invalid.setText("Error: You must enter the book id in the correct format.");
